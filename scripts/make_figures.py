...
for label, vals, color in segments:
    ...
    for i, v in enumerate(vals):
        if v:
            ax.text(
                i, bottoms[i] + v / 2, str(v), ha="center", va="center", color="white", fontsize=10, fontweight="bold"
            )
    bottoms = [b + v for b, v in zip(bottoms, vals)]

ax.set_ylabel("generations (n = 50 per language)")
ax.set_title(
    "How Qwen2.5-Coder 1.5B handles failure on failure-prone I/O tasks\n"
    "(measured; candidates are not verdicts — see article section 9)",
    fontsize=11,
)
ax.set_ylim(0, 55)
