...
BASEOPTS = {"temperature": 0.7, "num_ctx": 2048, "num_predict": 384, "num_thread": 4, "top_k": 40, "top_p": 0.9}
OUT = "raw7b"
...
total = len(tasks) * N
for tk in tasks:
    d = os.path.join(OUT, tk["language"])
    ...
    for seed in range(N):
        ...
        if not code.startswith("# GEN_ERROR"):
            ...
            if v == "swallow_cand":
                cands.append((f"{tk['id']}_s{seed}", note))
...
for lang in ("python", "typescript"):
    ...
    print(
        f"[{lang}] total={tot} unhandled={c['unhandled']} proper={c['proper']} swallow_cand={sw} parse_error={c['parse_error']}"
    )
