...
def main(d):
    ...
    if not files:
        print(f"no .py/.ts/.js files under {d}")
        return
    ...
    for f in files:
        try:
            ...
            unreadable += 1
            continue  # counted, not silently dropped (2026-09-19)
    ...
    if unreadable:
        print(f"  (unreadable, excluded from the denominator: {unreadable})")
    ...
    sem = None
    for cand in (
        os.path.join(HERE, ".venv", "Scripts", "semgrep.exe"),
        os.path.join(HERE, ".venv", "bin", "semgrep"),
        "semgrep",
    ):
    ...
    try:
        cfgs = [
            os.path.join(HERE, "rules", "silent-fallbacks-python.yml"),
    ...
    for f in cands:
        print(f"  {f}  [classifier: swallow_cand]")
        printed = True
    if not printed:
        print("  (none surfaced)")
    print(
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: PYTHONUTF8=1 python scripts/scan_mine.py <DIR>")
        sys.exit(2)
    main(sys.argv[1])
