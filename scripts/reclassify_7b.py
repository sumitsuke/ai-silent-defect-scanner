...
def cpy(s):
    ...
    for x in tries:
        for h in x.handlers:
            ...
            if op:
                v, note = "swallow_cand", "except:pass"
            elif rd and not hr and not hl:
                v, note = "swallow_cand", "return-default,no-log/raise"
    return v, note, doc
...
def cts(s):
    if "catch" not in s:
        return "no_try_except", "", False
    v, note = "proper", ""
    for m in CATCH.finditer(s):
        b = m.group(2).strip()
        if b == "":
            v, note = "swallow_cand", "empty-catch"
        elif (
...
for f in sorted(glob.glob("raw7b/**/*.*", recursive=True)):
    ...
    rows.append(
        dict(file=base, task=base.rsplit("_s", 1)[0], language=lang, classification=v, note=note, py_has_docstring=doc)
    )
...
print(
    "swallow_cand:",
    [(r["file"], r["note"], r["py_has_docstring"]) for r in rows if r["classification"] == "swallow_cand"],
)
