...
for scope, pred in [
    ("ALL", lambda f: True),
    ("FAILURE-PRONE", lambda f: "control" not in f),
    ("CONTROL", lambda f: "control" in f),
]:
    ...
    for lang in ("python", "typescript"):
        c = cnt[lang]
        tot = sum(c.values())
        h = tot - c["unhandled"] - c["parse_error"]
        sw = c["swallow_cand"]
        rate = f"{100 * sw / h:.0f}%" if h else "n/a"
        print(
            f"  [{lang}] n={tot} no_try_except={c['unhandled']} proper={c['proper']} swallow_cand={sw}  cond_swallow={sw}/{h}={rate}"
        )
