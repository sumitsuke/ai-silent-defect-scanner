...
def classify_py(src):
    ...
    for t in tries:
        for h in t.handlers:
            ...
            ret_default = any(
                isinstance(n, ast.Return) and n.value is not None and const_str(n.value) in DEFAULT_CONSTS for n in body
            )
            if only_pass:
                verdict = "swallow_cand"
                note = "except:pass"
            elif ret_default and not has_raise and not has_log:
                verdict = "swallow_cand"
                note = "return-default,no-log/raise"
    return verdict, note
...
def classify_ts(src):
    ...
    for m in CATCH.finditer(src):
        ...
        if body == "":
            ...
            re.search(r"return\s+(null|undefined|\[\]|\{\}|false|''|\"\")\s*;?", body)
            and "throw" not in body
            and not LOGY.search(body)
        ):
            verdict = "swallow_cand"
            note = "return-default,no-log/throw"
    return verdict, note
...
for lang in ("python", "typescript"):
    ...
    print(
        f"[{lang}] total={tot}  unhandled={c['unhandled']}  proper={c['proper']}  swallow_cand={c['swallow_cand']}  parse_error={c['parse_error']}"
    )
    sw = c["swallow_cand"]
    print(
        f"   handled={handled}  conditional swallow rate = swallow/(proper+swallow) = {sw}/{c['proper'] + sw} = {(100 * sw / (c['proper'] + sw) if (c['proper'] + sw) else 0):.0f}%"
    )
